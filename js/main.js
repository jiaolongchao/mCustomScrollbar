    ...
    function handleComplete(e) {
        ...
        setVal();
        init();
        $('.loading-layer').delay(1000).fadeOut(800, function() {
            $(this).remove();
        })
        hylink.trackPV('君越minisite-首页','/home');
    }
    ...
    $.get('main.html', function(data) {
        mainHtml = $(data);
        var imgs = mainHtml.find('img');
        imgs.each(function() {
            Manifest.push($(this).attr('src'))
        });
        queue.loadManifest(Manifest);
        hylink.trackPV('君越minisite-loading','/loading');
    }, 'html');
    ...
    function listenVideo() {
        ...
        v.on('timeupdate', function(event) {
            ...
            $vdesc.removeClass('show-description-mod active').hide();
            $('.v-mask-layer').hide();
    ...
    function setQiliu() {
        ...
        $('.fengbox').html('');
        $('.fengbox').spritespin({
            source: SpriteSpin.sourceArray('images/qiliu/{frame}.png', {
                frame: [1, 31],
                digits: 2
            }),
            width: 1400,
            ...
            animate: true,
            onLoad: function() {
                $('.fengbox').height('auto').width('100%')
                $('.fengbox canvas').css({ 'position': 'static', 'height': 'auto' })
            }
        });
    ...
    function setScroll() {
        _mcs = $(".content-wrap").mCustomScrollbar({
    ...
    function setCurNav() {
        for (var i = 0; i < $contentMod.length; i++) {
            if (getScrollTop() < 200) {
  

                $navs.removeClass('active');
                $('[data-nav="#' + $contentMod.eq(0).attr('id') + '"]').addClass('active');
                break;

            }else if (getScrollTop() >= $content.height() - $(window).height()-100) {
                $navs.removeClass('active');
                $('[data-nav="#media').addClass('active');
                                break;


            }else{
              if (checkIsView($contentMod.eq(i))) {
                    if (!$navs.eq(i).hasClass('active')) {
                        $navs.removeClass('active');
                        $('[data-nav="#' + $contentMod.eq(i).attr('id') + '"]').addClass('active');
                    }
                    break;
                }                
            }
    ...
    function checkfdjNum() {
        if (checkIsView($paramsBox)) {
            if (!$paramsBox.hasClass('in')) {
                $paramsBox.addClass('in');
                fdjNum();
            }
        } else {
            $paramsBox.removeClass('in');
        }

    };

    function fdjNum() {
        var f = curFdjData[0] == 192 ? 1 : 0;
        numAni({
            el: $fdjValue.eq(0),
            num: curFdjData[0],
            float: 0
    ...
    function bindEvent() {
        $('.ol a.ok').on('click', function(event) {
            event.preventDefault();
            $('.ol').hide();
        });
        $('.submit-form').on('submit', function(event) {
            event.preventDefault();
            var i = $('.submit-form').index(this);
            var _this = $(this);
             $('.has-error', _this).removeClass('has-error');
            if (_this.hasClass('active')) return;
            var realname = $('.realname', this).val();
            var mobile = $('.mobile', this).val();
            var privacy = $('.privacy', this).is(':checked');
            if (realname == '') {
                _this.find('.realname').siblings('b').text("您尚未输入真实姓名").parent().addClass('has-error');
                 _this.find('.realname').focus();
                adjust();

            } else if (!hylink.checkMobile(mobile)) {
                _this.find('.mobile').siblings('b').text("请输入正确的手机号码").parent().addClass('has-error');
                 _this.find('.mobile').focus();
                adjust();
            } else if (!privacy) {
                _this.find('.privacy').siblings('b').text("您需要同意隐私政策才能继续").parent().addClass('has-error');

            } else {
                _this.addClass('active');
                hylink.testdrive(function(data) {
                    _this.removeClass('active');
                    if (data.status == 'suc') {
                        $('.realname,.mobile', _this).val('')
                        if (i == 0) {
                            hylink.trackPV('君越minisite-预约试驾浮层-试驾成功浮层', '/testdrive-succeed');
                        } else {
                            hylink.trackPV('君越minisite-底部预约试驾-成功浮层', '/down-testdrive-succeed');
                        }
                        $('.ol').show();
                        pyBuickCvtPC('lacrosse', data.leadId, 'GM');

                    }
                }, realname, mobile, '全新一代君越');
            };
            if (i == 0) {
                hylink.trackEvent('君越minisite-预约试驾浮层-提交按钮', '预约试驾页面', '点击', '提交');
            } else {
                hylink.trackEvent('君越minisite-底部预约试驾-提交', '底部预约试驾', '点击', '提交');
            };

        });
        $('#showtest').on('click', function(event) {
            event.preventDefault();
            var _this = $(this);
            if (!_this.hasClass('active')) {
                _this.addClass('active');
                $poptd.show().stop().animate({ top: 54 }, 500, function() {
                    hylink.trackPV('君越minisite-预约试驾浮层', '/testdrive');
                });
            } else {
                $poptd.stop().animate({ top: -285 }, 500, function() {
                    $poptd.hide();
                    _this.removeClass('active');
                })
            }

            hylink.trackEvent('君越minisite-顶部-预约试驾按钮', '顶部', '点击', '预约试驾');
        });
        $('.close-td').on('click', function(event) {
            $poptd.stop().animate({ top: -285 }, 500, function() {
                $poptd.hide();
        ...
        $('.interior .hotpoint').on('click', function(event) {
            ...
            hylink.trackEvent('君越minisite-内饰-'+track[i], '内饰', '点击', track[i]);
            setTimeout(function(){
                    hylink.trackPV('君越minisite-内饰-'+track[i], ga[i]);
            },500);
        });
        ...
        $navs.on('click', function(event) {
            ...
            var txt = $(this).text();
            hylink.trackEvent('君越minisite-顶部-'+txt+'按钮', '顶部', '点击', txt);
        ...
        $('.sel-fdj-btn a').on('click', function(event) {
            event.preventDefault();
            var _this = $(this);
            if (_this.hasClass('active')) return;
            $('.sel-fdj-btn a').removeClass('active');
            _this.addClass('active');
            var i = $('.sel-fdj-btn a').index(this);
            $('.fdj-mod').stop().fadeOut(300);
            $('.fdj-mod-' + (i + 1)).stop().fadeIn(300);
            $('.fdj .copy').stop().fadeOut(300);
            $('.fdj .copy-' + (i + 1)).stop().fadeIn(300);
            curFdjData = fdjData['fdj' + i];
            if(i == 0){
                $('.param-box-1 p.kw').html('KW<br>/5400RPM')
                $('.param-box-3 p.kw').html('NM<br>/2000-5000RPM');
            }else{
                $('.param-box-1 p.kw').html('KW<br>/5600RPM')
                $('.param-box-3 p.kw').html('NM<br>/2000-4000RPM');
            }
            fdjNum();
            var track = ['发动机2.0T','发动机1.5T'];
            hylink.trackEvent('君越minisite-动力-'+track[i], '动力', '点击', track[i]);
        ...
        $('.exterior .feature-pic .btn').on('click', function() {
            var track = ['展翼型LED尾灯','展翼型LED大灯'];
            var _this = $(this);
            var i = parseInt(_this.closest('.item').data('index'));
            if (_this.hasClass('more')) {

            } else if (_this.hasClass('ty')) {
                _this.closest('.item').removeClass('light-mod');
                hylink.trackEvent('君越minisite-'+track[i]+'-白天','外观','点击',track[i]+'白天');

            } else if (_this.hasClass('yl')) {
                _this.closest('.item').addClass('light-mod');
                hylink.trackEvent('君越minisite-'+track[i]+'-夜晚','外观','点击',track[i]+'夜晚');
            };



        });
        $('.exterior .pic-con .more').on('click', function() {
            var i = $('.exterior .pic-con .more').index(this);
            showExteriorBig(i)

        });
        $('.exterior .pic-con .item').find('img.hp').on('click', function() {
            var i = $('.exterior .pic-con .item').index($(this).parent(".item"));
            showExteriorBig(i)

        });
        $('.car-list a').on('click', function() {
            var txt = $(this).text();
            hylink.trackEvent('君越minisite-底部别克车系-'+txt, '底部别克车系', '点击', txt);
        ...
        $('.show-carlist').hover(function() {
            carList.show().stop().animate({ 'right': 0 }, 500,function(){
                hylink.trackPV('君越minisite-底部别克车系', '/buickcar');
            })

        }, function() {
            carList.stop().animate({ 'right': -1096 }, 500, function() {
                $(this).hide();
            })
        });
        ...
        $('[data-video]').on('click', function(event) {
            var track = ['外观', '内饰', '动力', '安全']
            curcentV = parseInt($(this).attr('data-video'));
            currentData = videoTxt['t' + curcentV];
            var n = parseInt($videolayer.attr('data-video'));
            if (curcentV != n) {
                $videolayer.attr('data-video', curcentV);
                // v.src(videos[curcentV - 1]);
                v.src('http://www.buick.com.cn/getvideo/youku/getvideo.ashx?vid='+videos[curcentV - 1]+'&year=2016&type=mp4&iswebserice=1');
            }
            $vdesc.removeClass('show-description-mod active').hide();
            $('.v-mask-layer').hide();
            $videolayer.fadeIn(function() {
                v.play();
            });
            if(curcentV < 5){
                hylink.trackEvent('君越minisite-'+track[curcentV-1]+'-视频点击按钮', track[curcentV-1], '点击', '视频');
            }else{
                hylink.trackEvent('君越minisite-精彩鉴赏-精彩视频', '精彩鉴赏', '点击', '精彩视频');
            }
        });
        $vdesc.find('.show-desc').on('click', function(event) {
            v.pause();
            $vdesc.addClass('show-description-mod');
            $('.v-mask-layer').show();
        });
        $vdesc.find('.video-play').on('click', function(event) {
            v.play();
            $('.v-mask-layer').hide();
            $vdesc.removeClass('show-description-mod');
        });

        $('.buick-logo').on('click', function(event) {
            hylink.trackEvent('君越minisite-顶部-别克logo点击按钮', '顶部', '点击', '别克logo');
        });
        $('.buick-lacrosse').on('click', function(event) {
            hylink.trackEvent('君越minisite-顶部-君越logo点击按钮', '顶部', '点击', '君越logo');
        });
        ...
        $('#policy .article').on('click', function(event) {
            event.stopPropagation();
        });
        $resetBtn.on('click', function(event) {
            var form = $(this).closest('.submit-form');
            form[0].reset();
            hylink.trackEvent('君越minisite-预约试驾浮层-重置按钮', '预约试驾页面', '点击', '重置按钮');
        });
        $('.privacylabel a').on('click', function(event) {
            event.preventDefault();
            var i = $('.privacylabel a').index(this);
            if (i == 0) {
                hylink.trackEvent('君越minisite-顶部-隐私条款', '顶部', '点击', '隐私条款');
            }else{
                hylink.trackEvent('君越minisite-底部预约试驾-隐私政策', '底部预约试驾', '点击', '隐私条款');
            }
            $('#policy').fadeIn('fast', function() {
                if (i == 0) {
                    hylink.trackPV('君越minisite-顶部-隐私条款', '/secret');
                }else{
                    hylink.trackPV('君越minisite-底部预约试驾-隐私政策', '/down-secret');
                }
            });

        });
        $('.media .b-privacy a').on('click', function(event) {
            event.preventDefault();
            $('#policy').fadeIn('fast');
    ...
    function showExteriorBig(i) {
       var track = ['展翼型LED尾灯','展翼型LED大灯','全新飞翼式镀铬进气隔栅'];

        var pics = [
            'images/exterior/big/pic01.jpg',
            'images/exterior/big/pic02.jpg',
            'images/exterior/big/pic03.jpg'
        ]
        showPopLayer(pics, 1, i, setLight, function() {
            $popLayer.find('.btn').on('click', function() {
                if ($(this).hasClass('ty')) {
                    $(this).closest('li').removeClass('light-mod');
                    var j = parseInt($(this).data('index'))
                    hylink.trackEvent('君越minisite-'+track[i]+'-浮层-白天', '外观', '点击', track[i]+'浮层白天');
                } else {
                    $(this).closest('li').addClass('light-mod');
                    var j = parseInt($(this).data('index'))
                    hylink.trackEvent('君越minisite-'+track[i]+'-浮层-夜晚', '外观', '点击', track[i]+'浮层夜晚');
                }
            });

            hylink.trackPV('君越minisite-'+track[i]+'-浮层','exterior-'+(i+1));
        });

        hylink.trackEvent('君越minisite-'+track[i]+'-加号', '外观', '点击', track[i]+'展加号');
    ...
    function videoDesc() {
        if (curcentV > 4) return;
        var curtime = parseInt(v.currentTime());
        for (var i = 0; i < currentData.length; i++) {
            if (!$vdesc.hasClass('active')) {
                if (currentData[i].start == curtime) {
                    $vdesc.addClass('active').stop().fadeIn(100);
                    $vdesc.find('.title').text(currentData[i].title);
                    if (currentData[i].desc == '') {
                        $vdesc.find('.show-desc').hide();
                        $vdesc.addClass('nodesc')

                    } else {
                        $vdesc.find('.desc').text(currentData[i].desc);
                        $vdesc.find('.show-desc').removeAttr('style');
                        $vdesc.removeClass('nodesc');
                    }
                    break;
                }
            } else {
                if (currentData[i].end == curtime) {
                    $vdesc.removeClass('active').stop().fadeOut(100, function() {
                        $vdesc.removeClass('show-description-mod');
                    });
                    $('.v-mask-layer').hide();
                    break;
    ...
    function setLight() {
        var lis = $popLayer.find('li');
        for (var i = 0; i < lis.length; i++) {
            lis.eq(i).append('<img src="images/exterior/big/t' + (i + 1) + '.jpg">');
            if (i < lis.length - 1) {
                lis.eq(i).append('<img class="light" src="images/exterior/big/pic0' + (i + 1) + '-2.jpg"><div class="light-btn-box"><div data-index="'+i+'" class="btn ty"><img src="images/exterior/ty.png"></div><div class="btn yl" data-index="'+i+'"><img src="images/exterior/yl.png"></div></div>');
            }
        }
    };
    ...
    function createCookie(name, value, days) {
        ...
        document.cookie = name + "=" + value + expires + ";domain=.buick.com.cn;path=/";
    ...
    function checkPrivacyCookie() {
        if (readCookie('agree_policy') != null) {
            $("#privacy,#privacy2").attr('checked', true);
        };
        $("#privacy,#privacy2").on('change', function(event) {
            if ($(this).is(':checked')) {
                createCookie('agree_policy', 1, 30);
            } else {
                createCookie('agree_policy', null, -1);
            };
    ...
    function checkHash() {
        ...
        if(hash == '#spec'){
            $(".content-wrap").mCustomScrollbar('scrollTo', $('#spec').position().top, {
                scrollInertia: 0
            });

        }else if(hash == '#beauty'){
            $(".content-wrap").mCustomScrollbar('scrollTo', $('#media').position().top, {
                scrollInertia: 0
            });
   ...
   function adjust() {
        $('.has-error b').each(function() {
            $(this).css('left', ($(this).parent().width() - $(this).width() - 56) / 2);
        });
    }

    function pyBuickCvtPC(_type, _orderno, _campaign) {
        if (typeof(_orderno) == 'undefined') { _orderno = ''; }
        var pya = 'R6.6P.zmhzxXavhZKQhQoScPzaEP';
        switch (_type) {
            case 'enclave':
                pya = 'R6.3n.UdGOiCusdYFusouz4sXNAX';
                break;
            case 'envision':
                pya = 'R6.Ce.w6LmQH6KNXnbsKbf4Drda0';
                break;
            case 'encore':
                pya = 'R6.nH.bY2vnzce-gxUSMKHpjLkUP';
                break;
            case 'gl8':
                pya = 'R6.5n.M7crKieZogts2mSsVATbS_';
                break;
            case 'lacrosse':
                pya = 'R6.-A.JzxlLnb7FESQGYNdGe1b20';
                break;
            case 'newgs':
                pya = 'R6.cA.5JTnL337G_c5fo0LpB_7N_';
                break;
            case 'excellegt':
                pya = 'R6.zH.8_-3zw0t3wj2kWHgex97xP';
                break;
            case 'verano':
                pya = 'R6.5o.Og1DEp_oJsXNUXo6Z8L1jX';
                break; }
        var w = window,
            d = document,
